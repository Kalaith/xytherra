...
export const getGalaxyDimensions = (size: 'small' | 'medium' | 'large'): number => {
  return gameConstants.GALAXY.DIMENSIONS[
    size.toUpperCase() as keyof typeof gameConstants.GALAXY.DIMENSIONS
  ];
};

export const getSystemCount = (size: 'small' | 'medium' | 'large'): number => {
  return gameConstants.GALAXY.SYSTEM_COUNTS[
    size.toUpperCase() as keyof typeof gameConstants.GALAXY.SYSTEM_COUNTS
  ];
};
...
export const generateRandomPlanetCount = (): number => {
  return (
    Math.floor(Math.random() * gameConstants.PLANET.MAX_PER_SYSTEM) +
