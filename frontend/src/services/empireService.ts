...
export class EmpireService {
  ...
  private static checkResourceAlerts(empire: Empire): TurnProcessingResult['notifications'] {
    ...
    Object.entries(empire.resources).forEach(([resource, amount]) => {
      if (
        amount < resourceWarningThreshold &&
        empire.resourceIncome[resource as ResourceType] <= 0
      ) {
        alerts.push({
          type: 'warning',
    ...
    Object.entries(cost).forEach(([resource, amount]) => {
      if (amount && empire.resources[resource as ResourceType] < amount) {
        missingResources.push(
          `${resource}: need ${amount}, have ${Math.floor(empire.resources[resource as ResourceType])}`
        );
      }
    });
   ...
   */
  static deductResources(empire: Empire, cost: Partial<Record<ResourceType, number>>): Empire {
    const updatedEmpire = { ...empire };
   ...
   */
  static addResources(empire: Empire, resources: Partial<Record<ResourceType, number>>): Empire {
    const updatedEmpire = { ...empire };
