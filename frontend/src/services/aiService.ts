import type { Empire, GameState, AIPersonality, Notification } from '../types/game.d.ts';
...
export class AIService {
  ...
  private static makeResearchDecision(empire: Empire, _gameState: GameState): AIDecision | null {
    ...
    const basePriority =
      gameConstants.AI.DECISION_WEIGHTS[
        personality.toUpperCase() as keyof typeof gameConstants.AI.DECISION_WEIGHTS
      ].RESEARCH;
   ...
   */
  private static selectTechByPersonality(
    ...
    for (const domain of preferredDomains) {
      const domainTechs = availableTechs.filter(techId => TECHNOLOGIES[techId].domain === domain);
    ...
    if (personality === 'scientific') {
      const sortedByCost = availableTechs.sort(
        (a, b) => TECHNOLOGIES[b].cost - TECHNOLOGIES[a].cost
      );
  ...
  private static makeMilitaryDecision(empire: Empire, _gameState: GameState): AIDecision | null {
    ...
    const currentMilitaryPower = empire.fleets.reduce(
      (total, fleet) => total + fleet.ships.length,
      0
    );
  ...
  static calculateThreatLevel(empire: Empire, targetEmpire: Empire): number {
    const militaryPowerRatio =
      this.calculateMilitaryPower(empire) / Math.max(1, this.calculateMilitaryPower(targetEmpire));
  ...
  private static calculateMilitaryPower(empire: Empire): number {
    return empire.fleets.reduce(
      (total, fleet) => total + fleet.ships.length * gameConstants.FLEET.BASE_COMBAT_POWER_PER_SHIP,
      0
    );
  }
}
