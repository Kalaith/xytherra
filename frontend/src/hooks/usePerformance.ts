...
export const useGalaxyStatsSelector = () => {
  return useGameStore(state => {
    ...
    const totalPlanets = systems.reduce((count, system) => count + system.planets.length, 0);
    const colonizedPlanets = systems.reduce(
      (count, system) => count + system.planets.filter(p => p.colonizedBy).length,
      0
    );
...
export const useEmpireStats = (empire: Empire | null) => {
  return useMemo(() => {
    ...
    const totalFleetPower = empire.fleets.reduce(
      (power, fleet) =>
        power + fleet.ships.reduce((shipPower, ship) => shipPower + ship.stats.attack, 0),
      0
    );

    const resourceTotal = Object.values(empire.resources).reduce((sum, amount) => sum + amount, 0);

    const incomeTotal = Object.values(empire.resourceIncome).reduce(
...
export const useVisibleSystems = (playerEmpireId: string) => {
  return useGameStore(state => {
    const systems = Object.values(state.galaxy.systems);
    return systems.filter(system => system.discoveredBy.includes(playerEmpireId));
  });
};
...
export const useSystemDetails = (systemId: string, playerEmpireId: string) => {
  return useGameStore(state => {
    ...
    const surveyedPlanets = system.planets.filter(p => p.surveyedBy.includes(playerEmpireId));
    const colonizedPlanets = system.planets.filter(p => p.colonizedBy === playerEmpireId);
