          ...
          >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div
                  className={`inline-block px-3 py-1 rounded-full text-xs border ${getDomainColor(TECHNOLOGIES[playerEmpire.currentResearch].domain)}`}
                >
                  {TECHNOLOGIES[playerEmpire.currentResearch].domain.charAt(0).toUpperCase() +
                    TECHNOLOGIES[playerEmpire.currentResearch].domain.slice(1)}
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-white mb-1">
                  {Math.round(
                    (playerEmpire.researchProgress[playerEmpire.currentResearch] /
                      TECHNOLOGIES[playerEmpire.currentResearch].cost) *
                      100
                  )}
                  %
                </div>
                <div className="text-sm text-slate-400">
                  {Math.floor(playerEmpire.researchProgress[playerEmpire.currentResearch] || 0)} /{' '}
                  {TECHNOLOGIES[playerEmpire.currentResearch].cost}
                </div>
          ...
          {availableTechs.length === 0 ? (
            <div className="bg-slate-800 rounded-lg p-8 text-center">
              ...
              <p className="text-slate-500 text-sm mt-2">
                Discover new planets or complete current research to unlock more
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center space-x-1">
                      <div
                        className={`px-2 py-1 rounded text-xs border ${getDomainColor(tech.domain)}`}
                      >
                  ...
                  <div className="flex justify-between items-center">
                    <div
                      className={`px-3 py-1 rounded-full text-xs border ${getDomainColor(tech.domain)}`}
                    >
                      {tech.domain.charAt(0).toUpperCase() + tech.domain.slice(1)}
                    </div>
                    <div className="text-slate-400 text-sm">Cost: {tech.cost} 🔬</div>
                  </div>
                  ...
                  {tech.prerequisites.length > 0 && (
                    <div className="mt-2 text-xs text-slate-500">
                      Requires:{' '}
                      {tech.prerequisites
                        .map(prereq => TECHNOLOGIES[prereq]?.name || prereq)
                        .join(', ')}
                    </div>
                  )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              {researchedTechs.map(([techId, tech]) => (
                ...
                >
                  ...
                  <div
                    className={`inline-block px-2 py-1 rounded text-xs border ${getDomainColor(tech.domain)}`}
                  >
                    {tech.domain.charAt(0).toUpperCase() + tech.domain.slice(1)}
                  </div>
                </div>
