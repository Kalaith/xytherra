...
const DiplomacyView: React.FC = () => {
  ...
  const otherEmpires = Object.values(gameState.empires).filter(empire => 
  ...
  const getPersonalityDescription = (empire: Empire) => {
    ...
    const descriptions = {
      ...
      expansionist: 'Rapid territorial growth is their priority. Always seeking new worlds to claim.',
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4 text-sm text-slate-300">
              <div>Colonies: <span className="font-semibold">{playerEmpire.colonies.length}</span></div>
              <div>Technologies: <span className="font-semibold">{playerEmpire.technologies.size}</span></div>
              <div>Wars Won: <span className="font-semibold">{playerEmpire.totalWars}</span></div>
            </div>
          </div>
          <p className="text-slate-300 text-sm">
            Leading faction in the galactic community. Your diplomatic choices will shape the future of the galaxy.
          </p>
        </motion.div>
        ...
        <div className="space-y-6">
          ...
          {otherEmpires.length === 0 ? (
            <div className="bg-slate-800 rounded-lg p-8 text-center">
              ...
              <p className="text-slate-500 text-sm mt-2">Explore more systems to discover other civilizations</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-start space-x-4">
                        <div 
                          ...
                          style={{ backgroundColor: empire.color + '30', border: `2px solid ${empire.color}` }}
                        >
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mb-2">
                            <div className={`flex items-center px-3 py-1 rounded-full text-sm border ${getRelationColor(relation)}`}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      <div className="bg-slate-700/50 rounded p-3 text-center">
                        <div className="text-lg font-semibold text-white">{empire.colonies.length}</div>
                        <div className="text-xs text-slate-400">Colonies</div>
                      </div>
                      <div className="bg-slate-700/50 rounded p-3 text-center">
                        <div className="text-lg font-semibold text-white">{empire.technologies.size}</div>
                        <div className="text-xs text-slate-400">Technologies</div>
                      </div>
                      <div className="bg-slate-700/50 rounded p-3 text-center">
                        <div className="text-lg font-semibold text-white">{empire.fleets.length}</div>
