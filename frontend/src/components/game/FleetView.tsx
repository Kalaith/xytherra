...
const FleetView: React.FC = () => {
  ...
  const getFleetExperience = (fleet: Fleet): number => {
    return (
      fleet.ships.reduce((total, ship) => total + (ship.experience || 0), 0) /
      Math.max(fleet.ships.length, 1)
    );
  };
        ...
        {fleets.length === 0 ? (
          <div className="bg-slate-800 rounded-lg p-8 text-center">
            ...
            <p className="text-slate-400 mb-4">
              Build your first fleet to explore and defend your empire
            </p>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div
                        className={`px-3 py-1 rounded-full text-xs border ${rank.color} bg-opacity-20`}
                      >
                      ...
                      <div className="text-center">
                        <div className="flex items-center justify-center mb-1">
                          ...
                          <span className="text-white font-semibold">
                            {fleet.coordinates
                              ? `${Math.round(fleet.coordinates.x)}, ${Math.round(fleet.coordinates.y)}`
                              : 'Unknown'}
                          </span>
                        </div>
                    ...
                    <div className="flex space-x-2 text-xs">
                      {fleet.ships
                        .reduce(
                          ...
                          [] as Array<{ design: string; count: number }>
                        )
                        .map(({ design, count }) => (
                          <span
                            key={design}
                            className="bg-slate-700 px-2 py-1 rounded text-slate-300"
                          >
                            {count}x {design}
                          </span>
                        ))}
              ...
              {designMode ? (
                ...
                <div className="bg-slate-800 rounded-lg p-8 text-center">
                  ...
                  <p className="text-slate-400">
                    Choose a fleet to view details and issue commands
                  </p>
                </div>
              )}
            </div>
          ...
          {fleet.ships.map((ship, index) => (
            <div key={index} className="bg-slate-700 rounded p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-slate-400 text-xs">Exp: {ship.experience || 0}</div>
              </div>
